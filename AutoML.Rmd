...
```{r,echo=TRUE}
##
ames_fit<- lm(log(SalePrice)~`Overall Qual` +Neighborhood+`Gr Liv Area`, data=ames_raw_2008)
summary(ames_fit)
```
...
So the first thing to do, I suppose, would be to deal with the neighborhood data. There are a lot of neighborhoods in the indicator variable and condensing this might make some sense. I'll start by doing some analysis on how often each neighborhood appears in the dataset. 

```{r,echo=TRUE}
##
##
neighborhood_Freq<-ames_raw |> group_by(Neighborhood) |> count(Neighborhood)
ggplot(data= neighborhood_Freq, aes(x=Neighborhood, y=n))+
